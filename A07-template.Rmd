...
### Your code here
Trips <- Trips %>%
  mutate(duration = as.numeric(edate - sdate) / 3600) %>% # hours
  filter(duration < quantile(duration, 0.95)) # filter outliers

ggplot(Trips, aes(x = client, y = duration)) +
  geom_boxplot() +
  labs(title = "Rental Duration by Client Type",
       x = "Client Type",
       y = "Duration (hours)")
...
Trips %>%
  ...
  labs(title = "Bike Use by Day of the Year",
       x = "Day of the Year",
       y = "Density")
```


My observations :  Bike usage seems to peak around certain times of the year, potentially corresponding with weather changes or local events.
...
Trips %>%
  ...
  labs(title = "Bike Use by Day of the Week",
       x = "Day of the Week",
       y = "Density")
...
My observations :  Bike usage is higher on weekends, which might indicate recreational use.
...
Trips %>%
  ...
  labs(title = "Bike Use by Hour of the Day",
       x = "Hour of the Day",
       y = "Density")
...
My observations :There are two noticeable peaks in bike usage: one in the morning and one in the late afternoon, likely corresponding to commuting times.
...
### Your Code Here
Trips %>%
  mutate(minute_of_hour = minute(sdate)) %>%
  ggplot(aes(x = minute_of_hour)) +
  geom_density(fill = "purple", alpha = 0.5) +
  labs(title = "Bike Use by Minute of the Hour",
...
My observations : Bike checkouts are fairly evenly distributed across the minute of the hour, with slight peaks at certain intervals. 
...
Trips %>%
  ...
  labs(title = "Bike Rentals by Hour, Day of Week, and Client Type",
       x = "Hour of the Day",
       y = "Number of Rentals")
...
My comments : Regular users show more consistent usage throughout the week, while casual users show a higher spike on weekends.
...
# Density plot of station-to-station distances
ggplot(PairDistances, aes(x = distance)) +
  geom_density(fill = "orange", alpha = 0.5) +
  labs(title = "Density Plot of Station-to-Station Distances",
       x = "Distance (km)",
       y = "Density")

# Density plot of ride distances
ggplot(Trips, aes(x = distance)) +
  geom_density(fill = "cyan", alpha = 0.5) +
  labs(title = "Density Plot of Ride Distances",
       x = "Distance (km)",
       y = "Density")

# Extra credit: Both densities on the same plot
ggplot() +
  geom_density(data = PairDistances, aes(x = distance, fill = "Station-to-Station"), alpha = 0.5) +
  geom_density(data = Trips, aes(x = distance, fill = "Ride Distances"), alpha = 0.5) +
  labs(title = "Density Plots of Station-to-Station and Ride Distances",
       x = "Distance (km)",
       y = "Density") +
  scale_fill_manual(values = c("Station-to-Station" = "orange", "Ride Distances" = "cyan"))
```
...
Optional Extra credit answer here : Simple and Simple2 were given different variable names for the station to clearly distinguish between the start station (sstation) and the end station (estation). This is necessary for correctly calculating the distances between pairs of stations without confusion.
